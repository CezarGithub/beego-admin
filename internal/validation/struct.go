...
func ValidateStruct(structPtr interface{}, fields ...*FieldRules) error {
	...
	for i, fr := range fields {
		...
		//must check if field is a pointer to a Model
		m := fv.Elem().Interface()
		//check GetID method
		//fields of Model type : we check only ID=0.
		//to check all fileds just comment first if{} sentence including else word.
		if isModel, ok := m.(interface{ GetID() int64 }); ok {
			i := isModel.GetID()
			if i == 0 { //check only if ID=0
				errs[getErrorFieldName(ft)] = errors.New("validation.cannot_be_blank")
			}
			// regular field
		} else if err := Validate(fv.Elem().Interface(), fr.rules...); err != nil {
...
func findStructField(structValue reflect.Value, fieldValue reflect.Value) *reflect.StructField {
	...
	for i := structValue.NumField() - 1; i >= 0; i-- {
		...
		if sf.Anonymous || sf.Type.Kind() == reflect.Ptr { //reflect.Ptr for rel(fk) beego fields; anonymous for embedded fields ex.Base
